...
export const FinanceReviewModal = ({ data, onClose }) => {
  ...
  const handleReject = async () => {
    ...
    setIsSubmitting(true);
    try {
      const success = await updateFinanceQuoteApproval("Finance Rejected", remarks.trim());
      if (success) {
            ...
            <div className="p-6 border-b">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className={`p-4 rounded-lg ${
                  isOverBudget ? 'bg-red-50' : isUnderBudget ? 'bg-green-50' : 'bg-gray-50'
                }`}>
                  <p className="text-sm text-gray-600">Variance</p>
                  <div className="flex items-center space-x-2">
                    <p className={`text-2xl font-bold ${
                      isOverBudget ? 'text-red-600' : isUnderBudget ? 'text-green-600' : 'text-gray-600'
                    }`}>
                      {isOverBudget ? '+' : ''}{formatCurrency(Math.abs(budgetDifference))}
                    </p>
                    {isOverBudget && <Warning size={20} className="text-red-500" />}
                    {isUnderBudget && <CheckCircle size={20} className="text-green-500" />}
                  </div>
                  <p className={`text-sm ${
                    isOverBudget ? 'text-red-600' : isUnderBudget ? 'text-green-600' : 'text-gray-600'
                  }`}>
                    {variancePercentage}% {isOverBudget ? 'over' : isUnderBudget ? 'under' : ''} budget
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="p-6 border-b">
              ...
              <div className="space-y-4">
                {boq_items?.map((boqItem, index) => (
                  <div key={boqItem.boq_id || index} className="border rounded-lg overflow-hidden">
                    ...
                    <div className="p-4">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <h5 className="font-medium text-gray-700 mb-2">BOQ Details</h5>
                          <div className="space-y-1 text-sm">
                            <p>Unit Cost: {formatCurrency(boqItem.unit_cost)}</p>
                            <p>Total Cost: {formatCurrency(boqItem.total_cost)}</p>
                          </div>
                        </div>
                        
                        <div>
                          <h5 className="font-medium text-gray-700 mb-2">Supplier Quotes</h5>
                          {approved_supplier?.items
                            ?.filter(item => item.boq_id === boqItem.boq_id)
                            .map((quoteItem, quoteIndex) => (
                              <div key={quoteIndex} className="bg-green-50 p-3 rounded-lg mb-2">
                                <p className="text-sm font-medium text-green-800">
                                  {quoteItem.material_name}
                                </p>
                                <div className="flex justify-between text-sm text-green-700">
                                  <span>{formatCurrency(quoteItem.unit_price)} × {quoteItem.quantity} {quoteItem.unit}</span>
                            ...
                            <p className="text-sm text-gray-500">No matching supplier quotes</p>
                          )}
                        </div>
                      </div>
                    </div>
