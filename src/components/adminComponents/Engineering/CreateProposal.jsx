...
const CreateProposal = () => {
  ...
  return (
    <div className="flex gap-2 h-3/3">
      {/* Left Pane: Scrollable Leads List */}
      <div className="w-2/5 border-r pr-4 bg-white shadow px-4 py-2 rounded-md" style={{ maxHeight: '100%' }}>
        <h2 className="text-lg font-semibold mb-2">Select a Lead</h2>
        <div className="h-8/9 overflow-y-auto border-t">
           <ul className="space-y-2 my-4 ">
              {leads.map(lead => (
                <li
                  key={lead.id}
                  className={`p-4 rounded shadow cursor-pointer bg-gray-100 border ${selectedLead?.id === lead.id ? 'border-[#4c735c] bg-[#4c735c]' : 'hover:bg-[#4c735c]/10'}`}
      ...
      <div className="w-3/5 pl-6 flex flex-col h-3/3 overflow-y-auto bg-white shadow px-4 py-2 rounded-md">
        ...
        <div className="flex-grow overflow-y-auto border-t"> 
            ...
            {selectedLead ? (
              <form onSubmit={handleSubmit} className=" p-6 rounded space-y-4" encType="multipart/form-data">
                <h2 className="text-xl font-semibold">Proposal for <span className='text-[#4c735c] font-bold'>{selectedLead.client_name}</span></h2>
                <input
                  type="text"
                  name="title"
                ...
                <button type="submit" className="bg-[#4c735c] text-white px-4 py-2 rounded cursor-pointer">
                  Submit Proposal
                </button>
              </form>
            ) : (
              <div className="flex-1 flex items-center justify-center  text-gray-500 text-lg mt-24">Select a lead to create a proposal</div>
            )}
        </div>
      </div>
    </div>
