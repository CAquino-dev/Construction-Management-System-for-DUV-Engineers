...
export const MyProjectMilestoneUpdateStatus = ({ milestone, onClose }) => {
  ...
  const handleStatusUpdate = async () => {
    let nextStatus = null;
    if (milestone.progress_status === "Payment Confirmed")
      nextStatus = "In Progress";
    else if (milestone.progress_status === "In Progress") {
      toast.error("Please upload a completion photo to mark as Completed");
      return;
    } else {
      toast.error("No status update available");
      return;
    }
    ...
    try {
      const res = await fetch(
        ...
        }/api/project/project/milestones/${milestone.id}/status`,
        {
          method: "POST",
  ...
  const handleCompleteMilestone = async () => {
    if (!selectedImage) {
      toast.error("Please select an image as proof of completion");
      return;
    }
    ...
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_REACT_APP_API_URL}/api/engr/milestones/${
          milestone.id
        }/complete`,
        {
          method: "POST",
      ...
      <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-[800px] max-h-[90vh] overflow-auto flex flex-col">
        ...
        <div className="flex-grow">
          <h2 className="text-xl font-semibold mb-4">
            Update Status for Milestone "{milestone.status}"
          </h2>
          ...
          <div className="flex flex-row gap-4 items-center mb-4">
            <p className="text-gray-600">
              Status:{" "}
              <span
                className={`font-medium ${getStatusDotColor(
                  milestone.progress_status
                )}`}
              >
                {milestone.progress_status}
              </span>
            </p>
            {(milestone.progress_status === "Payment Confirmed" ||
              ...
              <>
                {milestone.progress_status === "In Progress" ? (
                  <button
                    ...
                    className={`bg-green-600 hover:bg-green-700 text-white px-5 py-1 rounded-md transition ${
