    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.userType === "Employee") {
          ...
          localStorage.setItem("userId", JSON.stringify(data.userId));
          navigate("/clientDashboard/projects-client");
        }
      } else {
      ...
      <div className="relative w-full max-w-md min-h-[500px] h-auto bg-white backdrop-blur-lg rounded-lg shadow-xl p-10 flex flex-col justify-center">
        <div className="flex flex-col items-center justify-center mb-4 gap-2">
          <img
            src={DUVLogoWhite}
            alt=""
            className="w-32 bg-[#4c735c] p-2 rounded mb-6"
          />
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <button
            type="submit"
            className="bg-[#4c735c] text-white py-3 rounded-lg hover:bg-[#4c735c]/90 cursor-pointer"
          >
            Login
          </button>
        </form>
        <p className="text-center mt-4">
          ...
          <a href="/register" className="text-[#4c735c]">
            Sign Up
          </a>
        </p>
      </div>
